...
## VOPRF-based Instantiation {#voprf-api}

In this section, we instantiate the high-level API in {{pp-api}} using the VOPRF
construction from {{I-D.irtf-cfrg-voprf}}. Note that we use the framework 
mentioned in the document to allow for batching multiple tokens into a single 
VOPRF evaluation. 

### Generate {#voprf-mapping-generate}
...
## Blind Signature Instantiation {#blind-sig-api}

In this section, we instantiate the high-level API in {{pp-api}} using the blind
signature construction from {{?I-D.irtf-cfrg-rsa-blind-signatures}}. This instantiation
does not support batching.

### Generate {#blind-rsa-mapping-generate}

The Generate functionality performs similar to that in {{voprf-mapping-generate}}.
Note that it also requires the server public key to run.

~~~
def Generate(m, com, pkS):
  inverses = []
  blindedMsgs = []
  for i in range(m):
    x = random_bytes() || com
    (blinded_msg, inv) = rsabssa_blind(pkS, x)
    inverses[i] = inv
    blindedMsgs[i] = blinded_msg
  return IssuanceInput {
           data: inverses,
           req: blindedMsgs,
         }
...
### Issue {#blind-rsa-mapping-issue}

This functionality only accepts a single blinded token input as `req`,
where `req` is the output of `rsabssa_blind`

~~~
def Issue(pkS, skS, req):
  blind_sig = rsabssa_blind_sign(skS, req)
  return IssuanceResponse {
           tokens: blind_sig,
           proof: nil,
         }
...
### Process {#blind-rsa-mapping-process}

Similar to `Issue`, this instantiation does not support batching.

~~~
Process(pkS, msg, inv, blind_sig):
  sig = rsabssa_finalize(pkS, msg, blind_sig, inv)
  rt = RedemptionToken { data: msg, issued: sig }
  return [rt]
~~~

### Redeem {#blind-rsa-mapping-redeem}
...
### Verify {#blind-rsa-mapping-verify}

Verify can be invoked by anyone holding the public key corresponding to the server
using the RSASSA-PSS-VERIFY procedure in {{!RFC3477}}.

~~~
def Verify(pkS, msg, sig):
